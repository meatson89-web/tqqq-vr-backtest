...
export const DEFAULT_SETTINGS = {
  ...
  // 예전에는 100%가 "한 주에 다 써버려 급반등 구간이 -0.8%로 꺾인다"는 이유로 60에
  // 머물렀는데, 추세국면 필터(regimeEnabled)를 켠 지금은 그 하방이 사라졌다. 200일선
  // 아래에서 POOL 지출이 멈추므로 하락 도중에 실탄을 소진하지 않는다.
  // 2026-09-02 데이터로 23창 재측정 (60% 대비):
  //   필터 ON  — 23창 평균 +2.7%, 독립 5창 +4.0%, 나빠진 창 0개, 실제 전체구간 +22.9%
  ...
  //       터지는 것"을 막는 게 목적이다.
  //   regimeBoostPct: 잠긴 동안 부스터가 걸린 주의 POOL 재투자 비율(평상시 100%).
