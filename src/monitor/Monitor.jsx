...
const WARNPERF = [
  ['과열복합 (이격도≥115 & RSI≥70)', '7/13', '146일', '-3.6%', '20%'],
  ['QQQ 200일선 하향돌파', '9/13', '94일', '-29.6%', '7%'],
  ['QQQ 50일선 하향돌파', '13/13', '111일', '-17.7%', '32%'],
  ...
  return (
    <div className="monitor">
      ...
      <div className="mn-rail">
        ...
        <Tile label="복합 과열신호" value={now.combo ? '발동' : '미발동'}
          tone={now.combo ? S3 : S2}
          sub={`이격도≥${bands.up_disp} AND RSI≥${bands.up_rsi} — 실측 최고효율 조합`} />
      </div>
      ...
      {tab === 'disp' && (
        <>
          ...
          <ResponsiveContainer width="100%" height={240}>
            <ComposedChart data={view} syncId="mn2" onClick={onClickChart} margin={{ top: 10, right: 18, left: 4, bottom: 0 }}>
              <CartesianGrid stroke={GRID} vertical={false} />
              <ReferenceArea y1={bands.up_disp} y2={200} fill={S3} fillOpacity={0.07} />
              <ReferenceArea y1={0} y2={bands.dn_disp} fill={S3} fillOpacity={0.07} />
              <XAxis {...XA} />
              <YAxis domain={['auto', 'auto']} width={64} tickFormatter={v => v + '%'}
        ...
        <>
          <div className="mn-finding">
            ...
            <p className="mn-note" style={{ margin: '0 0 10px' }}>
              {bands.n}번의 사이클 고점을 직전 6개월 QQQ 상승률에 대고 그린 결과입니다.
                ...
                <ResponsiveContainer key={k} width="100%" height={250}>
                  <ComposedChart margin={{ top: 20, right: 16, left: 4, bottom: 24 }}>
                    ...
                    <YAxis type="number" dataKey="y" domain={['dataMin-3', 'dataMax+3']} width={48}
                      tickFormatter={v => v.toFixed(0) + (k === 'd' ? '%' : '')}
                    ...
                    <ReferenceLine y={k === 'd' ? bands.up_disp : bands.up_rsi} stroke={S3} strokeOpacity={0.5}
                      label={{ value: `고정 ${k === 'd' ? bands.up_disp + '%' : bands.up_rsi}`, fill: S3, fontSize: 9, position: 'insideTopRight' }} />
                ...
                <tbody>
                  {[[0, '횡보'], [20, '보통'], [40, '급등']].map(([g, l]) => (
                    <tr key={g}><td>{l} (직전 6개월 {g > 0 ? '+' : ''}{g}%)</td>
                      <td style={{ color: S4, fontWeight: 600 }}>{(fit.d.a + fit.d.b * g).toFixed(1)}%</td>
            ...
            <p className="mn-note" style={{ margin: '12px 0 0' }}>
              ...
              예상 고점 이격도 <b>{fit.now_pred_d}%</b> / RSI <b>{fit.now_pred_r}</b>.
              현재 {fit.now_disp}%(<b style={{ color: fit.now_disp >= fit.now_pred_d ? S3 : S2 }}>{(fit.now_disp - fit.now_pred_d).toFixed(1)}%p</b>),
          ...
          <p className="mn-note" style={{ marginTop: 16 }}>
            ...
            과열복합은 꼭지 근처(-3.6%)에서 울리지만 절반만 잡고, -10% 룰은 전부 잡지만 이미 손실 뒤입니다.
          </p>
          <div className="mn-tw">
            <table>
          ...
          <tbody>
            {annual.map(r => (
              <tr key={r.y}>
                ...
                <td>{isFinite(r.dispMin) ? r.dispMin.toFixed(0) + '%' : '—'}</td>
                <td>{isFinite(r.dispMax) ? r.dispMax.toFixed(0) + '%' : '—'}</td>
