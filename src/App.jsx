  ...
  return (
    <div className="rules-panel info-panel-static">
      ...
      <div className="info-body">
        ...
        <p>
          <b>POOL 부스터</b> — 최근 N거래일(기본 252일 = 1년) 고점 대비 설정한 낙폭(기본 -30%) 이상
          하락한 주에는, 평소 5%였던 POOL 재투자 비율을 설정한 값(기본 100%)까지 올려서 급락 구간에
          더 공격적으로 재투자합니다.
          <b> 강도를 60%에서 100%로 올렸습니다</b> — 25%로는 발동해도 바닥에서 현금이 절반 남았습니다
          (2020-03-20 저점의 POOL 비중 55.3%). 60%면 18.5%까지 내려가고, 25~100% 전 구간이 단조롭게
          개선됩니다. 예전에는 100%가 한 주에 전액을 써버려 급반등 구간에서 꺾인다는 이유로 60%에
          머물렀는데, 추세국면 필터를 켠 지금은 그 단점이 사라졌습니다 — 200일선 아래에서는 POOL
          지출이 멈추므로 하락 도중에 실탄이 마르지 않습니다. 23창 재측정(2026-09-02 데이터)에서
          100%는 60% 대비 평균 <b>+2.7%</b>, 독립 5창 <b>+4.0%</b>, <b>나빠진 창 0개</b>,
          실제 전체구간 <b>+22.9%</b>였고 MDD(-66.6%→-66.9%)와 잔존율(83%)은 사실상 그대로입니다.
          다만 이 이점은 추세국면 필터가 켜져 있을 때만 유효합니다 — 끄면 23창 평균 -0.3%,
          최악 창 -9.3%로 뒤집힙니다.
          {' '}<b>검증(23창 기준)</b> — 끄기 대비 평균 <b>+20.9%</b>, 블록 95% 신뢰구간 [2.0, 45.9],
          23창 중 10개 개선·9개 차이 없음·악화 4개. 전체 구간은 567.7억 → 1880.8억(+231.3%)로
          이 화면에서 가장 효과가 큰 옵션입니다.
          <b> 다만 대가가 분명합니다</b> — MDD가 -53.0%에서 <b>-58.0%로 나빠집니다.</b> 바닥에서 현금을
          쏟아붓는 규칙이니 당연한 결과입니다. 그리고 국면을 심하게 탑니다: 실제 12창은 8개 개선인데
          합성 11창은 2개뿐이고, 독립 5창에서는 3/5입니다(닷컴 -1.6%, 금융위기 -4.8%, 2009~2014 +37.4%).
          {' '}<i>급락 뒤 빠른 반등</i>이 오는 국면에서만 값을 합니다.
        </p>
        <p>
          <b>기준 고점 기간을 60일에서 252일(1년)로 바꿨습니다 (2026-09-06).</b> 60일 고점은 긴 침체의
          바닥 다지기 국면에서 저절로 리셋됩니다 — 가격이 최근 고점 근처에 머물러 -30% 조건이 영영
          발동 불가가 됩니다. 1년 고점은 침체 전 고점이 남아 있어 조건이 계속 무장된 상태로 유지되고,
          추세국면 잠금이 풀리는 첫 주, 즉 복구 초입에서 POOL을 씁니다.
          {' '}<b>구조</b> — 252일 고점은 항상 60일 고점보다 크거나 같으므로, 이 변경은 발동 조건을
          <i> 추가만 하고 제거하지 않습니다</i>(합성·실제 전 구간에서 "60일은 무장인데 252일은 아닌 날"
          0건). 하방 경로는 "새로 산 날이 나빴다" 하나뿐이고, 그 경로는 추세국면 필터가 이미 막습니다.
          {' '}<b>근거</b> — 27년 합성 표본에서 252일에서만 무장하는 날은 807거래일(78개 구간)이고,
          그 날들의 전방 126일 수익률 중앙값은 <b>+33.5%</b>로 무조건 기저 +17.9%의 약 1.9배입니다.
          5년 롤링 창을 2주 간격 564창까지 촘촘하게 깔면 평균 <b>+2.0%</b>, 이긴 창 평균 +12.5% ·
          진 창 평균 -2.0%로 <b>손익비 약 6배</b>입니다.
          {' '}<b>믿지 말아야 할 부분</b> — 승률은 근거가 아닙니다. 23창에서 5승/3패로 보이지만
          격자를 촘촘하게 하면 <b>105승/97패/362무</b>, 사실상 동전던지기입니다. 평균이 양수인 것은
          순전히 손익비 덕입니다. 최악 창도 격자에 따라 -4.8%에서 <b>-10.7%</b>까지 벌어집니다.
          실제 TQQQ 데이터만 보면 16.5년 중 <b>2023-02-08 단 하루</b>의 의사결정이 전체구간 +18.4%의
          전부이고, 그 하루를 빼면 차이가 정확히 0입니다. 252일을 120일보다 선호할 근거는 합성 데이터의
          2009년 금융위기 재진입 2건뿐입니다(실제 데이터 47창에서 120일과 252일은 완전히 동일).
          블록 부트스트랩 95% 신뢰구간은 어떤 격자에서도 <b>0을 포함</b>합니다 — 통계적 유의성은 없고,
          이 변경은 "메커니즘과 사건 몇 건"에 대한 판단입니다. 되돌리려면 이 칸을 60으로 되돌리면 됩니다.
        </p>
        <p>
        ...
        <p>
          ...
          연속 경로(실제로 겪는 모습)는 실제 TQQQ 2010-2026 기준 <b>1178.96억/MDD -80.4% → 1275.39억/MDD -55.0%</b>,
          합성 1999-2026 기준 2233.17억 → 2582.20억(+15.6%)입니다. 반면 5년 창 23개로 세면
          <b>합계 -3.6%, 중앙값 -0.2%, 7승 12패</b>입니다. 2022가 없는 창에서는 일관되게 집니다
          (2016-02창 -17.0%, 2009-03창 -16.4%, 2017-02창 -15.9%). 창이 매번 1억으로 새로 시작해서
          2022를 피한 이득이 창 밖으로 못 넘어가기 때문입니다. <b>평시에 조금씩 내고 10년에 한 번
          크게 받는 보험</b>으로 보는 게 정확합니다.
        </p>
        <p>
          <b>잠금은 즉시, 해제는 확인 후.</b> 장기 하락은 200일선을 한 번만 넘지 않습니다. 즉시 해제로
          두면 2022년 하락 한 덩어리가 <b>6조각</b>으로 쪼개지고, 그 틈(2022-02-02 2거래일,
          2022-03-30 1거래일)에 <b>수요일이 정확히 2번</b> 낍니다. 그 주는 잠금이 풀려 부스터가
          살아 있었습니다. 실제로는 그날 부스터 조건이 꺼져 있어 유출이 없었지만, 그때 POOL이
          10.24억·9.73억이었으니 조건만 맞았으면 6억이 하락 도중에 나갔습니다 — 운이었지 설계가
          아니었습니다. 그래서 <b>해제 확인일수 3거래일</b>을 뒀습니다(1→6조각, 2→3조각, 3→1조각).
          진입 타이밍은 건드리지 않으므로 2022-01-26을 잡는 능력은 그대로입니다.
          다만 <b>어떤 값으로도 모든 약세장이 한 덩어리가 되지는 않습니다</b> — 2015-08~2016-07
          횡보장은 확인일수 10에서도 4조각입니다. 시장이 실제로 선 위아래를 오간 구간이라 그게 맞습니다.
        </p>
        <p>
          ...
          [-4.5, 12.7]로 0을 포함합니다. 27년 표본에서 이 규칙이 활약할 국면(POOL이 큰 상태에서의
          장기 하락)이 사실상 2022 한 번뿐이라 표본이 근본적으로 부족합니다.
        </p>
        <p>
          <b>추세국면 — 비용과 무효 구간.</b> 비용은 코로나형 V자 급락입니다. 2015-05~2020-05 창은
          12.90억 → <b>10.51억(-18.5%)</b>이 됩니다 — 2020-03~04의 바닥 매수가 잠겨서
  ...
  return (
    <div className="rules-panel param-panel">
      ...
      <p style={{ fontSize: 11, color: '#9ca3af', textAlign: 'left', margin: '4px 0 0' }}>
        ...
        주간 정액 적립금은 그대로 들어가고 <b>매도는 하지 않습니다</b>. 잠금은 <b>즉시 걸리고 해제만
        확인을 거칩니다</b> — 선 위로 올라와도 아래 일수만큼 유지돼야 풀립니다. 장기 하락이 200일선을
        넘나들며 여러 조각으로 쪼개지고, 그 틈에 낀 수요일에 부스터가 살아나는 것을 막기 위한 것입니다.
      ...
      <p style={{ fontSize: 11, color: '#9ca3af', textAlign: 'left', margin: '4px 0 0' }}>
        ...
        3이면 한 덩어리가 되고, 최종자산은 1 대비 0.3%만 줄어듭니다. 5·8도 한 덩어리지만 진짜 바닥
        매수가 밀려 수익이 각각 3.2%·5.7% 더 깎입니다.
      </p>
    ...
    <div className="rules-panel">
          ...
          <StatusItem
            ...
            value={rg.locked
              ? (rg.dwell > 0
                ...
                : `해제 대기 ${rg.aboveRun}/${rg.exitDays}일 · 아직 정지`)
      ...
      {rg.enabled && (
        <p style={{ color: '#9ca3af', fontSize: 12, textAlign: 'left', marginTop: 10 }}>
          {rg.locked
            ? (rg.dwell > 0
              ? `나스닥100(QQQ) 전일 종가 ${rg.close.toFixed(2)}가 ${settings.regimeMaLen}일선 ${rg.ma.toFixed(2)} 아래라 부스터와 POOL 재투자가 모두 정지된 상태입니다. 주간 정액 적립금은 그대로 들어갑니다. 선 위로 올라온 뒤 ${rg.exitDays}거래일 연속 유지돼야 해제되며, 복귀 후 가속 매수는 하지 않습니다.`
              : `선 위로 올라왔지만 아직 ${rg.aboveRun}/${rg.exitDays}거래일이라 잠금이 유지됩니다. ${rg.exitDays - rg.aboveRun}거래일 더 선 위에 있으면 해제됩니다. 하락 도중의 짧은 반등에 잠금이 풀려 부스터가 살아나는 것을 막기 위한 장치입니다.`)
            : `나스닥100(QQQ) 전일 종가 ${rg.close.toFixed(2)}가 ${settings.regimeMaLen}일선 ${rg.ma.toFixed(2)}보다 ${rg.gapPct.toFixed(1)}% 위라 국면 필터는 잠자입니다. 종가가 ${rg.ma.toFixed(2)} 아래로 내려가면 다음 날부터 부스터와 POOL 재투자가 멈춥니다(매도는 하지 않음). 이후 선 위로 올라와도 ${rg.exitDays}거래일 연속 유지돼야 풀립니다.`}
        </p>
      )}
      <p style={{ color: '#9ca3af', fontSize: 12, textAlign: 'left', marginTop: 10 }}>
        {s.boosterOn
